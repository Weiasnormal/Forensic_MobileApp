...
function buildTrendBuckets(
  ...
  cases.forEach((item) => {
    ...
    const bucket = buckets.find(
      (entry) =>
        createdAt >= entry.start.getTime() && createdAt <= entry.end.getTime(),
    );
    if (!bucket) return;
...
export default function AdminStatsScreen() {
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      suspectedPercent:
        totals.total > 0
          ? Math.round((totals.suspected / totals.total) * 100)
          : 0,
    };
  }, [filteredCases]);
          ...
          <View style={styles.rosterCard}>
            {topAnalysts.map((member, index) => (
              <TeamOverviewCard
                key={member.id}
                member={member}
                showDivider={index < topAnalysts.length - 1}
              />
            ))}
          </View>
        ) : (
...
function TrendLineChart({ buckets }: { buckets: TrendBucket[] }) {
  const maxValue = Math.max(
    1,
    ...buckets.flatMap((bucket) => [bucket.genuine, bucket.suspected]),
  );
  const stepX =
    buckets.length > 1
      ? (CHART_WIDTH - CHART_PAD_X * 2) / (buckets.length - 1)
      : 0;
  ...
  const toY = (value: number) =>
    CHART_HEIGHT -
    CHART_PAD_Y -
    (value / maxValue) * (CHART_HEIGHT - CHART_PAD_Y * 2);

  const genuinePoints = buckets
    .map((bucket, index) => `${toX(index)},${toY(bucket.genuine)}`)
    .join(" ");
  const suspectedPoints = buckets
    .map((bucket, index) => `${toX(index)},${toY(bucket.suspected)}`)
    .join(" ");
    ...
    <View>
      ...
      >
        <Polyline
          ...
          fill="none"
          stroke={colors.labelsuccess}
          strokeWidth={2.5}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <Polyline
          points={suspectedPoints}
          fill="none"
        ...
        {buckets.map((bucket, index) => (
          <React.Fragment key={`point-${index}`}>
            <Circle
              cx={toX(index)}
              cy={toY(bucket.genuine)}
              r={3.5}
              fill={colors.labelsuccess}
            />
            <Circle
              cx={toX(index)}
              cy={toY(bucket.suspected)}
              r={3.5}
              fill={colors.danger}
            />
          </React.Fragment>
