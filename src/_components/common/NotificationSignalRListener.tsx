...
export default function NotificationSignalRListener() {
	...
	useEffect(() => {
		...
		const handleMemberRequestStatus = (payload?: { message?: string; Message?: string }) => {
			useFeedbackStore.getState().showToast(
				payload?.message ?? payload?.Message ?? 'Your organization request status was updated.',
				'infoLight',
			);
		};
		...
		connection.onclose((error) => {
			if (error) {
				console.warn('[NotificationSignalRListener] Notification connection closed:', error.message);
				useFeedbackStore.getState().showToast(
		...
		void connection.start().catch((error) => {
			console.warn('[NotificationSignalRListener] Unable to connect to notification hub:', error);
		});
		...
		return () => {
			...
			if (connection.state !== HubConnectionState.Disconnected) {
				void connection.stop().catch((error) => {
					console.warn('[NotificationSignalRListener] Unable to stop notification connection:', error);
				});
			}
		};
