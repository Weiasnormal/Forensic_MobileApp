...
export function useDeepLinkVerification() {
  ...
  useEffect(() => {
    async function handleUrl(url: string | null) {
      ...
      const isVerifyLink =
        parsed.hostname === "verify-email" || parsed.path === "verify-email";
      ...
      const newEmail = parsed.queryParams?.newEmail;
      const verificationType = parsed.queryParams?.type;
      const linkedEmail =
        typeof newEmail === "string"
          ...
          : typeof email === "string"
            ...
            : null;
      const isEmailChangeLink =
        typeof verificationType === "string" &&
        verificationType.trim().toLowerCase() === "change-email";
      ...
      if (
        ...
        const result = await verifyEmailToken(userId, token, {
          type: isEmailChangeLink ? "change-email" : undefined,
          email: typeof email === "string" ? email : undefined,
          newEmail: typeof newEmail === "string" ? newEmail : undefined,
        });
        if (result.ok) {
          ...
          if (isEmailChange) {
            ...
            router.replace({
              ...
              params: linkedEmail ? { verifiedEmail: linkedEmail } : undefined,
            });
            return;
          }
        } else {
          ...
          useFeedbackStore
            .getState()
            .showToast("Verification link expired", "infoLight");
        }
      }
